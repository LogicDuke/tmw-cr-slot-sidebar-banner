...
class TMW_CR_Slot_Offer_Sync_Service {
    ...
    public static function normalize_offer( $offer ) {
        ...
        return array(
            ...
            'tracking_url'    => esc_url_raw( (string) self::pick_first_value( $offer, array( 'tracking_url', 'trackingLink', 'tracking_link', 'affiliate_url', 'affiliateLink', 'campaign_url', 'campaignUrl', 'click_url', 'clickUrl', 'offer_url', 'final_url', 'url' ) ) ),
            'status'          => sanitize_text_field( (string) ( $offer['status'] ?? '' ) ),
            'approval_status' => sanitize_text_field( (string) ( $offer['approval_status'] ?? '' ) ),
            'approved'        => self::normalize_boolean_string( $offer['approved'] ?? '' ),
            'is_approved'     => self::normalize_boolean_string( $offer['is_approved'] ?? '' ),
            'availability'    => sanitize_text_field( (string) ( $offer['availability'] ?? '' ) ),
